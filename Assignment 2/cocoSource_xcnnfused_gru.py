...
class ImageCaptionModel(nn.Module):
    ...
    def forward(self, cnn_features, x_tokens, is_train: bool, current_hidden_state=None) -> tuple:
        ...
        if current_hidden_state is None:
            ...
            initial_hidden_state = torch.zeros(self.num_rnn_layers, x_tokens.shape[0], self.hidden_state_sizes, device="cuda")
        else:
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_state_size, num_rnn_layers, cell_type='GRU'):
        ...
        for inp_size in input_size_list:
            self.cells.append(GRUCell(hidden_state_size, inp_size))
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        for i in range(sequence_length):
            ...
            layer_0_inp = torch.cat((input_tokens, processed_cnn_features), dim=1) # 1
            ...
            cur_logit = output_layer(hidden_state_update[self.num_rnn_layers-1,:,:])
