...
class ImageCaptionModel(nn.Module):
    ...
    def forward(self, cnn_features, x_tokens, is_train: bool, current_hidden_state=None) -> tuple:
        ...
        if current_hidden_state is None:
            ...
            initial_hidden_state = torch.zeros(self.num_rnn_layers, x_tokens.shape[0], self.hidden_state_sizes*2, device="cuda")
        else:
...
class RNN(nn.Module):
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        for i in range(sequence_length):
            ...
            layer_0_inp = torch.cat((input_tokens, processed_cnn_features), dim=1)
            ...
            for i in range(1, self.num_rnn_layers):        
                hidden_state_update[i,:,:] = self.cells[i](hidden_state_update[i-1,:,:], current_hidden_state[i,:,:])

            current_hidden_state = hidden_state_update

            cur_logit = output_layer(hidden_state_update[self.num_rnn_layers-1,:,self.hidden_state_size:])
            logits_sequence.append(cur_logit)
            tokens = torch.argmax(cur_logit, dim=1)
            ...
            if i < sequence_length - 1:
                if is_train:
                    ...
                    input_tokens = embedding_layer(tokens)  # embedding_layer(predictions)
